...
def gold_trade_features():
    ...
    return (
        ...
        .withColumn(
            "ema_12", F.avg("price").over(Window.partitionBy("symbol").orderBy("timestamp").rowsBetween(-11, 0))
        )
        .withColumn(
            "ema_26", F.avg("price").over(Window.partitionBy("symbol").orderBy("timestamp").rowsBetween(-25, 0))
        )
